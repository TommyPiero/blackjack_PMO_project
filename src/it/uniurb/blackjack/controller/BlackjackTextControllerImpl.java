...
public class BlackjackTextControllerImpl implements BlackjackTextController{
	...
	public BlackjackTextControllerImpl(final Blackjack blackjack, final BlackjackTextView view) {
	...
	// method that gets the values of bet and side bet from the view catching possible errors
	private void onBetConfirm() {
	...
	// method that put in communication view and model for a move
	...
	// method that asks for insurance
	...
	public void playGame() {
		...
		onConfirmSetup();
		boolean playAgain = false; // flag for the player if he wants to play another round
		...
		do {
			...
			// evaluating the insurance outcome if player is insured
			if (this.blackjack.getPlayer().isInsured() ) {
				this.view.showInsuranceOutcome(this.blackjack.getDealerHand().isBlackjack(), this.blackjack.verifyInsurance());
			}
